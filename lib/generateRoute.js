...
function formatOptimize(param,params){
    ...
    if(isWin){
        firstItem = pageNext.split('.')[0].split(/\\/g);
    }
    ...
    if(!thirdItem.length){
        ...
        if(endPath!='index'){
            pathStr=pathStr+'/'+endPath;
        }
        nameStr=nameStr+'-'+endPath;
    }
    
    let templateStr = result;
    ...
    templateStr= templateStr.replace(pathReg,'/'+pathStr)
    templateStr= templateStr.replace(nameReg,nameStr)
    templateStr= templateStr.replace(componentReg,param)  
    newRou.push(templateStr);
    return newRou
}
